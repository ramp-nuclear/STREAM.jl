...
@testset "VAL-01: Symmetric MTR — HeatDiffusion + two ChannelAndContacts" begin
    ...
    conns = [
        ...
        [
            connect(
                getproperty(hd, Symbol(:thermal_right, i)),
                getproperty(cac_r, Symbol(:thermal_left, i)),
            ) for i in 1:nz
        ]...,
    ]
    @named sys = compose(
        System(conns, t; name=:mtr_val01), pump_l, hx_l, cac_l, pump_r, hx_r, cac_r, hd
    )
...
@testset "VAL-02: Asymmetric MTR — right channel at 363.15 K inlet" begin
    ...
    @named sys = compose(
        System(conns, t; name=:mtr_val02), pump_l, hx_l, cac_l, pump_r, hx_r, cac_r, hd
    )
    ssys = mtkcompile(sys; fully_determined=false)
