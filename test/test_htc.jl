...
@testset "HTC models" begin
    ...
    @testset "property basis picks the evaluation temperature" begin
        @test property_temperature(AtFilm(), 90.0, 40.0) == 65.0
        @test property_temperature(AtBulk(), 90.0, 40.0) == 40.0
        ...
        # let a user control.
        h_film = DittusBoelter()(T_wall, T_bulk, ṁ, Dh, A, H2O)
        h_bulk = DittusBoelter(; basis=AtBulk())(T_wall, T_bulk, ṁ, Dh, A, H2O)
        ...
        for (basis, T_prop) in ((AtFilm(), 65.0), (AtBulk(), 40.0))
            Nu_hand = dittus_boelter(Re(H2O, T_prop, ṁ, A, Dh), Pr(H2O, T_prop))
            h_hand = Nu_hand * κ(H2O, T_prop) / Dh
            @test DittusBoelter(; basis=basis)(T_wall, T_bulk, ṁ, Dh, A, H2O) ≈ h_hand
        end

        # With no wall-to-bulk difference the two bases have to agree.
        @test DittusBoelter()(50.0, 50.0, ṁ, Dh, A, H2O) ≈
              DittusBoelter(; basis=AtBulk())(50.0, 50.0, ṁ, Dh, A, H2O)
    end

    @testset "NusseltHTC closes any correlation" begin
        # A bare (Re, Pr) correlation works: the trailing temperatures are absorbed.
        @test NusseltHTC(constant_Nusselt(; Nu=10.0))(T_wall, T_bulk, ṁ, Dh, A, H2O) ≈
              10.0 * κ(H2O, film_temperature(T_wall, T_bulk)) / Dh
        # A four-argument one sees the temperatures it asked for.
        seen = Float64[]
        record = (Re_v, Pr_v, Tw, Tb) -> (push!(seen, Tw, Tb); 5.0)
        NusseltHTC(record)(T_wall, T_bulk, ṁ, Dh, A, H2O)
        @test seen == [T_wall, T_bulk]
    end

    @testset "named constructors" begin
        @test ConstantNusselt(; Nu=8.235)(T_wall, T_bulk, ṁ, Dh, A, H2O) ≈
              8.235 * κ(H2O, 65.0) / Dh
        # Python STREAM evaluates its laminar branches at the bulk, so ours default there.
        @test FullyDevelopedLaminar(GEOM_MTR).basis isa AtBulk
        @test DevelopingLaminar(GEOM_MTR; develop_length=0.3).basis isa AtBulk
        @test DittusBoelter().basis isa AtFilm
        # A develop length that matters: it must not give the fully-developed answer.
        @test DevelopingLaminar(GEOM_MTR; develop_length=0.05)(T_wall, T_bulk, ṁ, Dh, A, H2O) !=
              FullyDevelopedLaminar(GEOM_MTR)(T_wall, T_bulk, ṁ, Dh, A, H2O)
    end

    @testset "user-defined models" begin
        mine = FunctionHTC((Tw, Tb, m, dh, a, liq) -> 1234.0)
        ...
        @test DittusBoelter()(T_wall, T_bulk, ṁ, Dh, A, H2O, 2e5) ==
              DittusBoelter()(T_wall, T_bulk, ṁ, Dh, A, H2O)

        # Subtyping HTC directly is the other way in.
        @test MaximalHTC(mine, DittusBoelter())(T_wall, T_bulk, ṁ, Dh, A, H2O) ==
              max(1234.0, DittusBoelter()(T_wall, T_bulk, ṁ, Dh, A, H2O))
        @test MaximalHTC(FunctionHTC((args...) -> 1.0),
                         FunctionHTC((args...) -> 7.0),
                         FunctionHTC((args...) -> 3.0))(T_wall, T_bulk, ṁ, Dh, A, H2O) == 7.0
    end

    @testset "Elenbaas natural convection" begin
        el = Elenbaas(GEOM_MTR)
        ...
        Nu_hand = elenbaas_nusselt(Ra_hand, GEOM_MTR.depth, GEOM_MTR.L)
        @test el(60.0, 40.0, 0.0, Dh, A, H2O) ≈ Nu_hand * κ(H2O, 40.0) / Dh
    end

    @testset "RegimeDependentHTC" begin
        lam = FunctionHTC((args...) -> 4.0)
        turb = FunctionHTC((args...) -> 100.0)
        rd = RegimeDependentHTC(; laminar=lam, turbulent=turb,
        ...
        Re_film = Re(H2O, film_temperature(T_wall, T_bulk), ṁ_at(1900.0), A, Dh)
        ...
        @testset "branches keep their own property basis" begin
            ...
            rd_real = RegimeDependentHTC(;
                laminar=FullyDevelopedLaminar(GEOM_MTR),
                turbulent=DittusBoelter(),
                geom=GEOM_MTR,
            )
            m_lam = ṁ_at(1000.0)
            @test rd_real(T_wall, T_bulk, m_lam, Dh, A, H2O) ≈
                  FullyDevelopedLaminar(GEOM_MTR)(T_wall, T_bulk, m_lam, Dh, A, H2O)
            m_turb = ṁ_at(8000.0)
            @test rd_real(T_wall, T_bulk, m_turb, Dh, A, H2O) ≈
                  DittusBoelter()(T_wall, T_bulk, m_turb, Dh, A, H2O)
        end

        @testset "natural convection takes over at Gr/Re² > 1" begin
            nc = FunctionHTC((args...) -> 999.0)
            rd_nc = RegimeDependentHTC(; laminar=lam, turbulent=turb, natural=nc,
...
@testset "a user-defined HTC drives a compiled channel" begin
    ...
    @named cac_u = ChannelAndContacts(;
        n=n, geometry=geom, htc=FunctionHTC((Tw, Tb, m, dh, a, liq) -> h_fixed)
    )
    ...
    Nu_expected = [h_fixed * geom.Dh / κ(H2O, film_temperature(T_wall_bc, T_c))
                   for T_c in T_cells]
