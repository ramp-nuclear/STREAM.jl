...
@testset "PointKinetics" begin
    ...
    @testset "TF-01..TF-03: Temperature Feedback Construction" begin
        ...
        @named fuel = HeatDiffusion(nz=3, nx=2, Lz=0.6, Lx=0.005, y=0.07,
        ...
        @testset "TF-02b: 1D vector per channel cell" begin
            @named pk = PointKinetics(ctrl_zero; temp_worth=Dict(ch => [-0.001,-0.002,-0.003,-0.004,-0.005]))
            unames = string.(ModelingToolkit.getname.(unknowns(pk)))
            @test any(n -> occursin("T_source_ch", n), unames)
            @test length(unknowns(pk)) == 7 + 5
        end
        ...
        @testset "TF-02c: 2D matrix for HeatDiffusion (3x2=6 cells)" begin
            @named pk = PointKinetics(ctrl_zero; temp_worth=Dict(fuel => fill(-0.002, 3, 2)))
            unames = string.(ModelingToolkit.getname.(unknowns(pk)))
        ...
        @testset "TF-02d: shape mismatch raises ArgumentError (vector)" begin
            @test_throws ArgumentError PointKinetics(ctrl_zero; name=:pk,
                                                      temp_worth=Dict(ch => [1.0, 2.0]))
        end
        ...
        @testset "TF-02e: shape mismatch raises ArgumentError (matrix)" begin
            @test_throws ArgumentError PointKinetics(ctrl_zero; name=:pk,
                                                      temp_worth=Dict(fuel => fill(0.0, 2, 2)))
        end
        ...
        @testset "TF-03b: ref_temp missing key — constructor succeeds" begin
            ...
            @test_nowarn PointKinetics(ctrl_zero; name=:pk,
    ...
    @testset "TF-06: reactivity observable includes feedback" begin
        ...
        @named cac  = ChannelAndContacts(n=n, geometry=geom_tf,
                                          htc_correlation=constant_Nusselt(Nu=8.235),
                                          friction_correlation=laminar_friction(0.0025/0.070))
        @named fuel = HeatDiffusion(nz=n, nx=2, Lz=0.6, Lx=0.005, y=0.07,
                                     rho_s=19300.0, cp_s=116.0, k_s=174.0,
                                     power_shape=ps_tf)
        ...
        @named pk = PointKinetics(ctrl_tf6; rho_val=0.0,
    ...
    @testset "TF-07: strong negative feedback bounds power (analytical)" begin
        ...
        @named cac7  = ChannelAndContacts(n=n, geometry=geom_tf7,
                                           htc_correlation=constant_Nusselt(Nu=8.235),
                                           friction_correlation=laminar_friction(0.0025/0.070))
        @named fuel7 = HeatDiffusion(nz=n, nx=2, Lz=0.6, Lx=0.005, y=0.07,
                                      rho_s=19300.0, cp_s=116.0, k_s=174.0,
                                      power_shape=ps_tf7)
        ...
        @named pk7 = PointKinetics(step_ctrl; rho_val=0.0,
        ...
        full7 = compose_systems(rods7, pk7, pump_tf7, hx_tf7; connections=all_eqs7, name=:core7)
