...
// ---------------------------------------------------------------------------
// Phase 66 D-01..D-04: Structured codegen output
//
// generateCode returns CodeSection[] instead of `string`. Each section carries
// an ordered list of CodeSubBlock; each sub-block knows the canvas-node UUIDs
// that produced its lines (sourceIds) so the CodePreview component can light
// up the originating nodes on hover/click. The string form (still consumed by
// CodePreview/Toolbar/Export) is produced by serializeSections below.
// ---------------------------------------------------------------------------

export type CodeSectionName =
  | "Imports"
  | "Resources"
  | "Components"
  | "Composition"
...
/**
 * Internal helper. Section-scoped builder used by generateCode. The walker
 * pushes one CodeSubBlock per emission unit (one @named, one connect, one
 * helper call, …) so the CodePreview UI can hover/click at sub-block
 * granularity per D-01..D-04.
 */
class SectionBuilder {
  private sections: Map<CodeSectionName, CodeSubBlock[]> = new Map();
  private order: CodeSectionName[] = [
    "Imports",
    "Resources",
    "Components",
    "Composition",
  ...
  push(
    section: CodeSectionName,
    kind: CodeSubBlockKind,
    lines: string[],
    sourceIds: string[] = [],
  ): void {
    const subBlocks = this.sections.get(section)!;
    subBlocks.push({ lines, sourceIds, kind });
  }

  build(): CodeSection[] {
    const out: CodeSection[] = [];
    for (const name of this.order) {
      const subBlocks = this.sections.get(name)!;
      if (subBlocks.length === 0) continue;
      out.push({ name, subBlocks });
    }
    return out;
  }
}
...
/**
 * D-12 formatting floor: assemble CodeSection[] back into the single Julia
 * string consumed by CodePreview/Toolbar/Export.
 *
 * Rules:
 *   - Section header per non-empty section: `# === <Section Name> ===`
 *   - Exactly one blank line between adjacent sub-blocks within a section
 *   - Exactly one blank line between top-level sections
 *   - No trailing whitespace on any line
 *   - Output ends with a single trailing newline (current behavior preserved)
 */
export function serializeSections(sections: CodeSection[]): string {
  const sectionChunks: string[] = [];
  for (const section of sections) {
    if (section.subBlocks.length === 0) continue;
    const subChunks: string[] = [];
    for (const sub of section.subBlocks) {
      subChunks.push(sub.lines.join("\n"));
    }
    const body = subChunks.join("\n\n");
    sectionChunks.push(`# === ${section.name} ===\n${body}`);
  }
  const assembled = sectionChunks.join("\n\n");
...
export function generateCode(
  ...
  // Empty canvas — emit a single-sub-block Imports section so the
  // CodeSection[] contract holds even on empty canvas. The serializer renders
  // this as a `# === Imports ===` header followed by the placeholder comment.
  if (nodes.length === 0) {
    return [
      {
        name: "Imports",
        subBlocks: [
          {
            lines: ["# Add components to the canvas to generate Julia code."],
            sourceIds: [],
            kind: "comment",
          },
        ],
  ...
  // --- Imports section (D-04: single sub-block, sourceIds=[]) ---
  // The smoke header + every `using ...` line are one logical sub-block.
  const importLines: string[] = [];
  importLines.push(
    "# =============================================================================",
  );
  importLines.push("# Generated by STREAM Composer");
  importLines.push(
    "# =============================================================================",
  );
  importLines.push("using ModelingToolkit, STREAM");
  importLines.push("using ModelingToolkit: t_nounits as t");
  ...
  if (hasFileLoadedShape || hasFileBCMode) {
    importLines.push(
      "using DelimitedFiles  # for file_loaded power shapes / file BC profiles",
    );
  }
  sb.push("Imports", "import", importLines, []);
  ...
  if (resources) {
    ...
    // --- Geometries (D-03: one sub-block per Geometry, sourceIds=[geom.uuid]) ---
    for (const g of geometries) {
      geomNameByUuid.set(g.uuid, g.name);
      const geomLines: string[] = [];
      // Pitfall 4: collision warning lives on the same sub-block as the
      // declaration it warns about (hover-highlights the same resource).
      ...
      if (geomLines.length > 0) {
        sb.push("Resources", "resource", geomLines, [g.uuid]);
      }
    }

    // --- Power Shapes — per-consumer emission (D-03: kind:'consumer-ps',
    // sourceIds=[ps.uuid, hd.uuid]; sentinel/unknown ps -> sourceIds=[hd.uuid] only) ---
    for (const node of hdNodes) {
      ...
      // No ref at all -> emit a missing-ref warning sub-block + skip the variable
      if (psRef === undefined) {
        sb.push(
          "Resources",
          "consumer-ps",
          [`# WARNING: power_shape_ref missing on ${hdName}`],
          [node.id],
        );
        continue;
      }
      ...
      switch (psResource.kind) {
        case "uniform":
          psLines.push(`${varName} = ones(${nz}, ${nx})`);
          break;
        case "z_cosine": {
  ...
  // --- Components section (D-04: one sub-block per @named, sourceIds=[node.uuid]) ---
  for (const node of nodes) {
    ...
    const declText = emitComponentDeclaration(data, component, resolveRef);
    sb.push("Components", "component", declText.split("\n"), [node.id]);
  }

  // --- Thermal assembly declarations (Composition: kind:'helper',
  // sourceIds = union of CAC + HD UUIDs the helper consumes) ---
  if (hasThermalAssemblies) {
    for (const asm of assemblies) {
      const helperLines: string[] = [];
      // Check nz/n mismatch — emit NOTE in the same sub-block as the helper
      // call it warns about.
      ...
      const helperSourceIds: string[] = [];
      for (const cac of asm.cacEntries) helperSourceIds.push(cac.nodeId);
      helperSourceIds.push(asm.hdNodeId);

      if (asm.type === "symmetric_plate") {
        const cac = asm.cacEntries[0];
        helperLines.push(`# Thermal assembly (auto-detected: symmetric_plate)`);
        helperLines.push(
          `@named ${asm.assemblyName} = symmetric_plate(${cac.instanceName}, ${asm.hdInstanceName})`,
        );
      } else if (asm.type === "plate") {
        const chLeft = asm.cacEntries[0];
        const chRight = asm.cacEntries[1];
        helperLines.push(`# Thermal assembly (auto-detected: plate)`);
        helperLines.push(
        ...
        // unknown
        helperLines.push(`# TODO: verify thermal wiring`);
      }

      sb.push("Composition", "helper", helperLines, helperSourceIds);
    }
  }
  ...
  // Pre-eqs pass: emit profile-vars and function stubs as Components-section
  // bc-preeq sub-blocks (D-04, Plan 02 placement rationale: BC pre-eqs co-locate
  // with the @named declarations they bind, not in Resources). The ad-hoc
  // `# ---` header is dropped; the section header is now rendered by
  // serializeSections.
  ...
  for (const node of nodes) {
    ...
    const nRaw = data.parameters["n"];
    const nValue: string | number = typeof nRaw === "number" ? nRaw : "n";
    const item: SourceEmitItem = {
      instanceName: data.instanceName,
      paramName: valueParam.name,
      nodeId: node.id,
      entry: v,
      nValue,
    };
  ...
  for (const item of sourceEmitPlan) {
    ...
    if (e.preset === "cosine") {
      ...
      sb.push(
        ...
        "bc-preeq",
        [
          `${item.profileVarName} = cosine_T_wall_profile(${item.nValue}; amplitude=${formatReal(e.amplitude)}, peaking_factor=${formatReal(e.peakingFactor)})`,
        ],
        [item.nodeId],
      );
      ...
      sb.push(
        ...
        "bc-preeq",
        [
          `${item.profileVarName} = rebin_intensive(readdlm(joinpath(@__DIR__, ${JSON.stringify(e.path)}), ','), ${item.nValue})`,
        ],
        [item.nodeId],
      );
    }
  ...
  for (const item of sourceEmitPlan) {
    ...
    sb.push(
      ...
      "bc-preeq",
      [
        `${e.functionName}(${argList}) = 0.0  # TODO: define your time-varying source value`,
      ],
      [item.nodeId],
    );
  }

  // --- Composition section: eqs = [ ... ] block ---
  // The opener / closer (`eqs = [` / `]`) are each their own sub-block so the
  // connect/anchor/binding sub-blocks between them stay grouped under
  // Composition with the D-12 single-blank-line discipline. Julia accepts
  // blank lines inside an array literal with trailing commas, so the emitted
  // Julia stays valid.
  sb.push("Composition", "comment", ["eqs = ["], []);

  // Flow connections (Composition: one sub-block per connect, kind:'connect',
  // sourceIds=[source.uuid, target.uuid]).
  ...
  // Each edge is one sub-block; for per-cell array connects the comment header
  // and the for-comprehension line collapse into one logical sub-block.
  for (const edge of thermalEdgesNonAssembly) {
    ...
    if (sourceIsArray || targetIsArray) {
      ...
      sb.push(
        ...
        "connect",
        [
          `    # ConstantTemperature per-cell connections`,
  ...
  // Pressure anchors (D-05) — Composition: kind:'anchor', sourceIds=[node.uuid].
  ...
  // BC bindings — Composition: kind:'bc-binding', sourceIds=[consumer.uuid]
  // (plus the source-node uuid in source mode). The legacy `# BCs for X:` line
  // is dropped: with sub-block-level rendering, the consumer-node grouping is
  // already visible via the bc-binding sub-blocks' sourceIds. A redundant
  // inline comment would clutter the array literal.
  for (const item of bcEmitPlan) {
    const {
      consumerData,
      consumerNode,
    ...
    if (entry === undefined || entry.mode === "mark") {
      ...
      const todoLines = targets.map(
        (tgt) => `    # TODO: set ${consumerData.instanceName}.${tgt}[i] here`,
      );
      sb.push("Composition", "bc-binding", todoLines, [consumerNode.id]);
      continue;
    }

    if (entry.mode === "value") {
      const bindLines = targets.map(
        (tgt) =>
          `    [${consumerData.instanceName}.${tgt}[i] ~ ${formatReal(entry.value)} for i in 1:${nValue}]...,`,
      );
      sb.push("Composition", "bc-binding", bindLines, [consumerNode.id]);
      continue;
    }
    ...
    if (entry.mode === "source") {
      ...
      if (!sourcePort) {
        sb.push(
          ...
          [consumerNode.id, entry.sourceNodeId],
        );
        continue;
      }
      const bindLines = targets.map(
  ...
  // Composition closer: `]` (paired with the `eqs = [` opener above).
  sb.push("Composition", "comment", ["]"], []);

  // --- Main section (D-04: single sub-block, sourceIds=[]) ---
  ...
  if (hasThermalAssemblies) {
    ...
    mainLines.push(`@named sys = ODESystem(eqs, t; systems=[${systemsList}])`);
  }
  mainLines.push("ssys = mtkcompile(sys)");

  // Solve stub stays inside the Main sub-block so the @named sys + mtkcompile
  // + solve-comment cluster reads as one logical block in the preview.
  mainLines.push("");
  mainLines.push("# Solve (uncomment to run)");
  mainLines.push("# sol = solve(SteadyStateProblem(ssys, []), DynamicSS(Rodas5P()))");

  sb.push("Main", "system", mainLines, []);
