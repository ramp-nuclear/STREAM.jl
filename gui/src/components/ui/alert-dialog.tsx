...
// Phase 72 — primitive-layer recommit. Mirrors Dialog:
//   - bg-background → bg-popover, rounded-lg → rounded-md
//   - shadow-lg → shadow-[var(--shadow-dialog)]
//   - bg-black/50 scrim → bg-foreground/40
//   - 100 ms fade-in / 80 ms fade-out, no zoom
//   - text-lg title → text-title, text-sm description → text-body opacity-65
//   - size-16 media circle uses bg-card (not bg-muted) for tonal consistency
...
function AlertDialogTitle({
  ...
  return (
    <AlertDialogPrimitive.Title
      ...
      className={cn(
        "text-title leading-tight font-medium sm:group-data-[size=default]/alert-dialog-content:group-has-data-[slot=alert-dialog-media]/alert-dialog-content:col-start-2",
        className
      )}
      {...props}
    />
...
function AlertDialogMedia({
  ...
  return (
    <div
      ...
      className={cn(
        "mb-2 inline-flex size-16 items-center justify-center rounded-md bg-card sm:group-data-[size=default]/alert-dialog-content:row-span-2 *:[svg:not([class*='size-'])]:size-8 [&_svg]:stroke-[1.5]",
        className
      )}
      {...props}
    />
