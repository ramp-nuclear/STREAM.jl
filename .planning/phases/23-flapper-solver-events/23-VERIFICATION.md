...
|----|----------------------------------------------------------------------------------------------------|------------|-------------------------------------------------------------------------------------------|
| 1  | `Flapper()` returns an MTK System with two FlowPorts and a continuous event                        | VERIFIED   | `src/components/flapper.jl` line 75: `compose(System(..., continuous_events=[cb]), port_in, port_out)` |
...
| 4  | The C1 Hermite ramp `3*xi^2 - 2*xi^3` interpolates R_closed to R_open                             | VERIFIED   | Line 69: `(R_closed + (R_open - R_closed) * (3 * xi^2 - 2 * xi^3)) * port_in.mdot`        |
...
|-----------------------------|---------------------------------|----------------------------------------------|----------|----------------------------------------------------------------------------|
| `src/components/flapper.jl` | `src/connectors.jl`             | `FlowPort` constructor (`@named port_in`)    | WIRED    | Lines 51–52: `@named port_in = FlowPort()`, `@named port_out = FlowPort()` |
...
| FLAP-04     | 23-01-PLAN.md    | User wires trigger via `flapper.ref_mdot ~ reference_component.port_in.mdot`                      | SATISFIED | `flapper.jl` line 72 comment; `test_flapper.jl` line 23 and 91 wire it    |
