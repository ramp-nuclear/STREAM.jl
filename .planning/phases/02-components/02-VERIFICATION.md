...
| 2 | `Pump(dP_pump=...)` instantiates and imposes constant pressure rise across FlowPorts | VERIFIED | `port_out.P - port_in.P ~ dP_pump` (line 92); public kwarg is `dP_pump` (line 84); ROADMAP SC-2 uses `dP_pump=` — exact match |
...
| 7 | 02-03 | Correct pressure equations for all three | VERIFIED | Pump: `port_out.P - port_in.P ~ dP_pump` (line 92); Friction: Blasius + Darcy-Weisbach (lines 115-117); Gravity: `rho_water(T_in) * 9.80665 * H` (line 134) |
...
| Friction pressure equation | `rho_water, mu_water` via `T_in` | `T_in = instream(port_in.T)` | `rho_water(T_in)` (line 117), `mu_water(T_in)` (line 114) | WIRED |
...
| COMP-02 | 02-01, 02-03, 02-04 | Pump — constant pressure rise, FlowPort in/out; public API uses `dP_pump` kwarg | SATISFIED | `port_out.P - port_in.P ~ dP_pump` (line 92); `function Pump(; name, dP_pump)` (line 84); 2 active tests |
...
- **Pump:** `port_out.P - port_in.P ~ dP_pump` pressure rise; public kwarg `dP_pump` matches MTK parameter name exactly; mass continuity and isenthalpic pass-through present.
