...
result: issue
reported: "Owner does not want the topology-hint chip feature at all. Separately: the handle design has multiple visual problems — amber/orange color, diamond shape, connector size, movement behavior, and looks horrible when FlowPort and thermal handles end up on the same side of a node. These are design concerns, not routing bugs."
severity: cosmetic
...
- truth: "Handle design should be visually clean — correct color, shape, size, and behavior when FlowPort and thermal handles share a node side"
  status: failed
  reason: "User reported: amber/orange color unwanted, diamond shape unwanted, connector size wrong, movement behavior wrong, looks horrible when FlowPort and thermal handles are on the same side. Topology-hint chip feature also unwanted — remove entirely."
  severity: cosmetic
  test: 5
  root_cause: ""
  artifacts: []
