...
**Success Criteria** (what must be TRUE):
  ...
  2. User can connect Pump.outlet to Channel.inlet with a visible edge
...
**Success Criteria** (what must be TRUE):
  ...
  4. BC panel allows adding `pump.inlet.P ~ 1.0e5`; generated code includes it
**Plans:** 3/3 plans complete
...
**Success Criteria** (what must be TRUE):
  1. A Channel with inlet unconnected shows a red/orange visual indicator on the canvas node
...
**Success Criteria** (what must be TRUE):
  ...
  6. FlowPort inlet and outlet handles display distinct polarity colors
