...
- `instream(outlet.T)` is the correct MTK idiom for the reverse-flow boundary inlet
...
- `inlet.T ~ T[1]` replaces `inlet.T ~ instream(outlet.T)` in ALL three channel variants
- `outlet.T ~ T[n]` unchanged
...
- `v[i]` stays signed: `inlet.mdot / (rho * A)`
- `velocity[i]` changed to unsigned: `abs(inlet.mdot) / (rho * A)` — speed magnitude
...
|----|-------------|-----------------|
| SIGN-01 | Channel handles negative `mdot` — Re uses `abs(mdot)`, friction dP flips sign, temperature advection upwinds correctly | Upwind fix in Channel energy loop (channel.jl:62-68) + inlet.T fix (channel.jl:91); dP formula already sign-correct via `mdot * abs(mdot)` |
...
| SIGN-03 | ChannelHeatFlux handles negative `mdot` | Upwind fix in ChannelHeatFlux energy loop (thermal_channel.jl:208-213); inlet.T fix propagates via `_channel_base_eqs` |
...
Phase 20 is a targeted bug-fix and test phase. There are exactly two defects to correct: a wrong port stream equation (`inlet.T ~ instream(outlet.T)` should be `inlet.T ~ T[1]`) and a forward-only upwind formula that produces incorrect temperature profiles under reverse flow. Both defects are in the energy balance and port-wiring sections of channel.jl and thermal_channel.jl. No new components, no new exports, no solver changes.

The fix strategy uses the project's established `ifelse()` idiom, already present in the dP formula (`mdot * abs(mdot)`) and the `regime_dependent` correlation switcher. Adding a second `instream(outlet.T)` call alongside the existing `instream(inlet.T)` provides the reverse-flow boundary temperature; per-cell upwinding then selects forward or reverse upstream temperature based on the sign of `inlet.mdot`. The `_channel_base_eqs` helper is the single fix point for the inlet.T equation — it propagates to both ChannelAndContacts and ChannelHeatFlux automatically; Channel's constructor has its own copy.

The test strategy uses `Pump(mdot0=-X)` to force a negative mass flow in a minimal closed loop. The existing `HeatExchanger` (temperature boundary reset at inlet side) provides the inlet temperature for the reversed-flow inlet via `instream(outlet.T)`. Three testsets — one per channel variant — each assert a monotonically reversed temperature profile, positive Re, and energy balance closure to 1% rtol.
...
T_inlet_fwd = instream(inlet.T)   # already present (was T_inlet)
T_inlet_rev = instream(outlet.T)  # NEW — reverse-flow boundary inlet
...
**What:** Replace `inlet.T ~ instream(outlet.T)` with `inlet.T ~ T[1]`.

**Why:** MTK stream semantics: `port.T` is the *outflow* temperature from this component through that port. Under reverse flow, fluid exits through inlet at the temperature of the first cell, T[1]. The old equation was wrong but silent because under forward flow, inlet.T gets zero weight in `instream()` computations (max(-mdot, 0) = 0 when mdot > 0).
...
**Why this works:** `Pump(mdot0=-X)` injects `inlet.mdot ~ -X` into the system. The channel will see a negative mdot at inlet throughout the steady-state solve. The `HeatExchanger` (or a bare T pin) on the reverse-flow inlet side sets the temperature entering through outlet via `instream(outlet.T)`.

**Topology for Channel test:**
```julia
# Pump -> HeatExchanger -> Channel -> back to Pump
# But flow direction is reversed: fluid enters Channel at outlet
...
connections = [
    ...
    pump.inlet.P ~ 1.0e5,
    ch.thermal.T   ~ T_wall,
    ch.inlet.T   ~ T_inlet,   # still needed for mtk symmetry
]
```

Under negative mdot the physical inlet is outlet (connected to pump.inlet via bc). The `instream(outlet.T)` call in the upwind formula will return the temperature from the upstream junction.

**Assertions after solve:**
```julia
T_vals = [sol[ssys.ch.T[i]] for i in 1:n]
# Under reverse heating: fluid enters hot end (outlet side = T[n]),
# cools toward T[1]. But if heating is applied (T_wall > T_inlet),
# and flow enters cold (T_inlet < T_wall) through outlet...
...
Wait — the CONTEXT.md test assertion is `T[1] < T[2] < ... < T[n]`. This means the temperature *increases* from cell 1 to cell n even under reversed flow. This is physically correct when the reverse-flow inlet (through outlet) is at the *cold* temperature `T_inlet`: fluid enters cold at cell n, is heated by the wall as it flows toward cell 1, so T[n] < T[n-1] < ... < T[1]. The assertion `T[1] < T[2]` would be WRONG in that case. Let me re-read the CONTEXT.md assertion carefully.

CONTEXT.md says: `T[1] < T[2] < ... < T[n]` with the label "temperature decreases axially under reverse heating". This is a contradiction in notation — if T[1] < T[n], temperature *increases* axially. The physical meaning is: reverse heating warms the fluid as it moves from inlet (outlet, cell n) to outlet (inlet, cell 1), so T increases from n toward 1 (i.e., T[n] < T[n-1] < ... < T[1]). The CONTEXT.md notation `T[1] < T[2] < ... < T[n]` would mean the profile is increasing left-to-right, which is the SAME as forward flow with T_wall > T_inlet.
...
- **Adding a `T_up_rev` parameter to `_channel_base_eqs`:** The function does not need to know about T_up — that is the caller's responsibility. Only the `inlet.T ~ T[1]` fix goes into `_channel_base_eqs`.
...
### Pitfall 2: The `inlet.T ~ T[1]` Fix Not Applied to Channel's Own Copy
...
**What goes wrong:** Incorrect boundary condition for the reversed-flow test — `instream(inlet.T)` is used for both forward and reverse, so the cold inlet temperature is wrong.

**Why it happens:** `T_inlet = instream(inlet.T)` was the only boundary call before the fix.

**How to avoid:** Explicitly declare `T_inlet_rev = instream(outlet.T)` alongside `T_inlet_fwd = instream(inlet.T)` before the loop. Use `T_inlet_rev` only in the `T_up_rev` branch of the i==n boundary condition.
...
- Physical flow direction: outlet side → inlet side (cell n → cell 1)
...
T_inlet_fwd = instream(inlet.T)   # renamed from T_inlet
T_inlet_rev = instream(outlet.T)  # NEW
...
push!(eqs, inlet.T                    ~ T[1])   # WAS: instream(outlet.T)
```
...
push!(obs, v[i]        ~ inlet.mdot / (rho_water(T[i]) * A))        # signed, unchanged
push!(obs, velocity[i] ~ abs(inlet.mdot) / (rho_water(T[i]) * A))   # unsigned, CHANGED
```
...
|--------------|------------------|--------------|--------|
| `inlet.T ~ instream(outlet.T)` (silent bug) | `inlet.T ~ T[1]` (correct outflow equation) | Phase 20 | Enables correct junction mixing under reverse flow |
...
| `velocity[i] ~ inlet.mdot / (rho * A)` (signed) | `velocity[i] ~ abs(inlet.mdot) / (rho * A)` (unsigned speed) | Phase 20 | Satisfies SIGN-02 "physically meaningful" |
...
2. **ChannelHeatFlux energy balance loop exact line numbers**
   - What we know: thermal_channel.jl lines 208-213 contain the ChannelHeatFlux energy loop; `T_inlet = instream(inlet.T)` at line 200
   - What's unclear: No ambiguity — reading the source confirmed this
   - Recommendation: Confirmed: ChannelHeatFlux has `T_inlet = instream(inlet.T)` at line 200 and energy loop at lines 208-215. Same fix pattern applies.
...
- Direct source code reading: `src/components/channel.jl` — exact line numbers for both inlet.T bug locations (line 91 in Channel, line 156 in `_channel_base_eqs`) and energy balance loop (lines 61-76)
...
- Fix correctness (`inlet.T ~ T[1]`): HIGH — stream semantics confirmed via connectors.jl + MTK documentation in CLAUDE.md
